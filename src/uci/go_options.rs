...
use crate::{uci::tokens::UCIGoToken::{self, *}, search::deadline::Deadline};
...
impl GoOptions {
    ...
    pub fn to_deadline(&self, board: &Board) -> Deadline {
        ...
        match self {
            ...
            GoOptions::TimeLimit { white_time, black_time, white_inc, black_inc } => {
                let pieces = board.combined().popcnt();
                let side = board.side_to_move();

                match side {
                    chess::Color::White => {
                        Deadline::timeout((*white_time - BUFFER) / pieces + *white_inc)
                    },
                    chess::Color::Black => {
                        Deadline::timeout((*black_time - BUFFER) / pieces + *black_inc)
                    },
                }
